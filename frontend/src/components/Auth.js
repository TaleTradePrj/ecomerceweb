...
export default function Auth(props) {
  ...
  return (
    <section className="flex w-screen h-screen flex justify-center md:items-center">
        <div className="flex flex-col w-[25rem] h-[44.625rem] md:m-20 justify-start items-center md:border border-gray-700/10 rounded-md pt-10">
            ...
            <input className="w-4/5 h-[3.25rem] border rounded-lg border-gray-700/10 outline-0 p-3 mt-5 hover:border-[#6254f3] focus:border-[#6254f3] bg-transparent text-gray-700 placeholder-gray-400 font-sans" placeholder="Email address" type="email"></input>
            <div className={`flex w-4/5 h-[3.25rem] mt-5 justify-evenly items-center border rounded-lg ${passInput ? "border-[#6254f3]" : "border-gray-700/10"} hover:border-[#6254f3] font-sans`}>
              <input type={isPassHidden ? "password" : "text"} className="ms-2 h-full w-4/5 outline-0" onFocus={() => {setPassInput(!passInput)}} onBlur={() => {setPassInput(!passInput)}} placeholder="Password"/>
            ...
            <button className="w-4/5 h-[3.25rem] bg-[#6254f3] text-white rounded-md mt-5 hover:bg-[#6254f3]/90 font-sans">{isLogin === "login" ? "Login" : "Sign up"}</button>
